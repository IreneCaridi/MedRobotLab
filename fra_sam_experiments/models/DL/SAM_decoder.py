...
from .Rep_ViT import LayerNorm2d
from .transformers import TwoWayTransformer
...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        transformer: nn.Module = TwoWayTransformer(depth=2, embedding_dim=256, mlp_dim=2048, num_heads=8, lora=False),
    ...
    def predict_masks(
        ...
        sparse_prompt_embeddings: torch.Tensor,  # COMMENT
        dense_prompt_embeddings: torch.Tensor,  # COMMENT
        num_prompts=None,
        kd_targets=None
        ...
        tokens = torch.cat((output_tokens, sparse_prompt_embeddings), dim=1)   # COMMENT

        # Expand per-image data in batch direction to be per-mask
        src = torch.repeat_interleave(image_embeddings, tokens.shape[0], dim=0)
        src = src + dense_prompt_embeddings  # COMMENT
...
#   TYPES OF PROMPTS
#
# point_coords : (torch.Tensor) Batched point prompts for this image, with shape BxNx2. Already transformed to the
...
# function to pos process masks TO BE ADJUSTED (was methods of SAM) (pass as self.encoder.img_size -> args.reshape_size)
# may be that it process masks out of just one img so WATCH-OUT
